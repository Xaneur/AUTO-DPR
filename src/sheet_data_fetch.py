...
def get_available_sheets(file_path: str) -> List[str]:
    ...
    try:
        ...
        sheets = [
            sheet.strip() for sheet in wb.sheetnames if sheet.strip() != "LOGS"
        ]  # Remove empty or whitespace-only names
        wb.close()
...
def get_descriptions_with_index(file_path, sheet_name="July.25"):
    ...
    """
    logger.info(
        f"getting descriptions from workbook from path : {file_path} and sheet name is : {sheet_name}"
    )
...
def get_date_column(file_path, sheet_name="July.25", date: datetime.date = None):
    ...
    """
    logger.info(
        f"getting date column from workbook from path : {file_path} and sheet name is : {sheet_name}"
    )
    wb = openpyxl.load_workbook(file_path)
    ws = wb[sheet_name]
        ...
        if isinstance(cell.value, datetime.datetime):
            if date is None:
                if cell.value.date() == datetime.date.today():
                    logger.info(
                        f"date column is : {cell.column} and col name is : {get_column_letter(cell.column)}"
                    )
    ...
    try:
        ...
        if cell.value is not None:
            try:
                ...
                logger.warning(
                    f"Existing value '{cell.value}' in cell {row_index},{column_index} is not a number. Treating as 0."
                )
        ...
        wb.save(file_path)
        logger.info(
            f"Successfully updated cell {row_index},{column_index} with value: {new_value} (previous: {current_value}, added: {value})"
        )
